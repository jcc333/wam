...
module Interpret = 
  ...
  //    let mutable trailPointer: List<Term> = []

  //    static member empty = Substitution(Map.empty)

  //    member this.Update(k, v) =
  //      ignore(trailPointer <- k :: trailPointer)
  //      this.TryFind(k)
  ...
  //      ignore(trailPointer <- if trailPointer.IsEmpty then [] else trailPointer.Tail)
  //      this.TryFind(k)
  ...
  //type Unification = Result<Substitution, Substitution>
  ...
  //type Frame = { Query: Query; Depth: int; Register: string; Predicate: Pred; TrailPointer: string }

  //type BStack = List<Frame>
  ...
  //  
  //  member this.tryEval(s: Substitution, goals: Query, kf: BStack): Substitution =
  //    match goals with
  //    | [] -> this.Apply(kf, s)
  //    | g::gs ->
  //      let Clause(h, b) as cls = freshClause n c
  //      let trailPtr = s.TrailPointer
  //      let frame = {Query = gs; Depth = n; Register = pg; Predicate = cs; TrailPointer = trailPtr}
  //      mgu(s, List.append b gs, n, pg, h, frame :: kf)

  //  member this.eval (s: Substitution) (q: Query) (n: int) (kf: BStack): Substitution =
  //    match q with
  //    | []                   -> s
  //    | (Var _) :: _         -> Answer.Error "Variable as goal"
  //    | App(o, _) as g :: gs ->
  //      let pg = sprintf("TOP%d", n)
  //      let s0 = s.Update(pg, g)
  //      tryEval(s0, gs, n, pg, (predicate s) kf)

  //  member this.mgu(s: Substitution, gs: Query, n: int, pg: String, t: Term, kf: BStack) =
  //    match ... with
  //    | Success(s0)  -> eval s0 gs (n + 1) kf
  //    | Failure(s0) -> this.Apply kf s0

  //  member this.Apply(stack: BStack, sub: Substitution) =
  //    match stack with
  //    | [] -> Answer.Error "No"
  //    | { Query = goals; Depth = depth; Register = register; Predicate = clauses; TrailPointer = trailPtr } :: onFail ->
  //       tryEval(sub.Unwind(Var trailPtr), goals, depth, register, clauses, failContinuation)


  //  //let ask q = eval Substitution("BOTTOM", ⊥) q 0 []

  //  //mostGeneralUnifier : Substitution → Query → Integer → VarN → Term → BStack → Substitution
  //  member this.DB 
